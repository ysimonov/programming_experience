...
// g++ CHALLENGE_download_images.cpp -lcurl -lboost_thread -lboost_system
...
size_t parallel_image_downloader(int num_images) {
    ...
    for (int i = 1; i <= num_images; i++) {
        boost::asio::post(thread_pool, [&i, &total_bytes]() {
            auto downloaded_bytes = download_image(i);
            std::unique_lock<std::mutex> lock(mutex_);
            total_bytes += downloaded_bytes;
        });
    }

    // // *** alternative solution using futures ***
    // std::list<std::future<size_t>> download_futures;
    // for (int i = 1; i <= num_images; i++) {
    //     download_futures.emplace_back(
    //         std::move(std::async(std::launch::async, download_image, i)));
    // }

    // while (!download_futures.empty()) {
...
int main() {
    ...
    size_t parallel_result =
        parallel_image_downloader(NUM_IMAGES);  // "warm up"
    for (int i = 0; i < NUM_EVAL_RUNS; i++) {
        auto startTime = std::chrono::high_resolution_clock::now();
