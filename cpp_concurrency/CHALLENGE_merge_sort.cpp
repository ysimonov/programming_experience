...
void parallel_merge_sort(int* array, unsigned int left, unsigned int right,
    ...
    if (left < right) {
        ...
        if (recursion_depth > DEFAULT_RECURSION_DEPTH) {  // sequential
            ...
            // auto left_future =
            //     std::async(std::launch::async, parallel_merge_sort, array,
            //     left,
            //                mid, recursion_depth + 1);  // sort the left half

            std::thread left_thread =
                std::thread(parallel_merge_sort, array, left, mid,
                            recursion_depth +
                                1);  // sort the left half in a separate thread
            parallel_merge_sort(array, mid + 1, right,
                                recursion_depth + 1);  // sort the right half

            // left_future.get();  // synchronize right half with the left half
...
void merge(int* array, unsigned int left, unsigned int mid,
    ...
    unsigned int num_left =
        mid - left + 1;  // number of elements in left subarray
    unsigned int num_right =
        right - mid;  // number of elements in right subarray
    ...
    unsigned int index_insert =
        left;  // index to insert elements into input array
